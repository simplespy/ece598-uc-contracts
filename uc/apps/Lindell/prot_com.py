...
class Commitment_Prot(UCProtocol):
    ...
    def env_commit(self, to_commit):
        ...
        if self.iscommitter and self.state == 0:
            self.msg = to_commit
            hash_value = self.write_and_wait_for('p2f', ('hash_to_group', to_commit), 'f2p')[0]
            ...
            self.commitment2 = (secp.ser(u2[0]), secp.ser(u2[1]), secp.ser(e2), secp.ser(v2))

            
            self.commitment = (u1[0], u1[1], e1, v1)
            self.write('p2f', ('sendmsg', self.receiver, ('commit', C1)))
            self.state = 1
        else:
            self.pump.write('')

    def env_reveal(self, eps=None):
        if self.iscommitter and self.state == 1:
            self.k2 = secp.uint256_from_str(os.urandom(32))
            k1 = hash((self.msg, self.commitment2))
            cp = self.g * k1 + self.z * self.k2
            self.write( 'p2f', ('sendmsg', self.receiver, ('open1', self.msg, cp)) )
            self.state = 2
        
        elif self.iscommitter and self.state == 2:
            
            (r, s) = self.randomness
            z = s + eps * r
            self.write( 'p2f', ('sendmsg', self.receiver, ('open2', self.commitment2, self.k2, z)) )

    def check_commit_1(self, preimage, ped):
        
        eps = secp.uint256_from_str(os.urandom(32))
        self.write('p2f', ('sendmsg', self.committer, ('response1', eps)))
        return (preimage, ped, eps)
        

    def check_commit_2(self, C2, k2, z, preimage, ped, eps):
        m = self.write_and_wait_for('p2f', ('value',), 'f2p')[0]
        self.g = m[0]
        self.z = m[1]
        self.g1 = m[2]
        ...
        hash_value = self.write_and_wait_for('p2f', ('hash_to_group', preimage), 'f2p')[0]
    ...
    def func_receive(self, fro, msg):
        if not self.iscommitter and self.state == 0 and msg[0] == 'commit':
            ...
            self.state = 1
        elif not self.iscommitter and self.state == 1 and msg[0] == 'open1':
            self.open1 = self.check_commit_1(msg[1], msg[2])
            self.state = 2
        elif not self.iscommitter and self.state == 2 and msg[0] == 'open2':
            preimage, ped, eps =  self.open1
            self.check_commit_2(msg[1], msg[2], msg[3], preimage, ped, eps)
            self.state = 3
        elif self.iscommitter and self.state == 2 and msg[0] == 'response1':
            self.env_reveal(msg[1])
        else:
