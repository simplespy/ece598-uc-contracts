...
class F_MCRS(UCFunctionality):
    def __init__(self, k, bits, crupt, sid, channels, pump):
        ...
        self.p_msg = lambda x: x[1][0]
        self.p_parse = lambda x: (x[0], *x[1][1:])

    def value(self, senderpid, senderssid):
        ...
        self.write( 'f2p', (senderpid, (senderssid, (self.output_value,))) )

    def sendmsg(self, senderpid, senderssid, to, msg):
        self.write(
            ch='f2p',
            msg=(to, (senderssid, ('recvmsg', senderpid, msg)))
        )
        
    def hash_to_group(self, sender, senderssid, msg):
        self.hash_table[msg] = self.hash_table.setdefault(msg, make_random_point(lambda x: self.sample(8*x).to_bytes(x,'little') ))
        self.write( 'f2p', (sender, (senderssid, (self.hash_table[msg],))))
